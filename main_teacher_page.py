...
class Button_other_materials(Button):
    def __init__(self, group, filename, base, command2=None, *args, **kw):
        super().__init__(*args, **kw)
    ...
    def delete(self):
        ...
        self.command2()
    def download(self):
        if download_data(self.group, self.base, self.filename)== "Error":
            self.config(text="file doesn't exist")
    def download2(self):
        if download_data(self.group, self.base, self.filename, answer=1) == "Error":
            self.config(text = "file doesn't exist")

class Indexed_button(Button):
    def __init__(self, filename, group, month, day, year, id, command2=None, *args, **kw):
        super().__init__(*args,**kw)
        self.index=0
        self.year = year
        self.group=group
    ...
    def set_mark(self):
        ...
        self.mark_text= ScrolledText(self.markwindow.root, width= 50, height=8 )
        self.markwindow.add_widgetpack(self.mark_text, side=TOP)
        self.mark_text.insert("0.0", get_mark(self.group, self.year, self.month, self.day, self.filename))
        def send():
            set_mark(self.group, self.year, self.month, self.day, self.mark_text.get("0.0", END), self.filename)
            self.markwindow.root.destroy()
        self.button_send_mark = Button(self.markwindow.root, text="send", command=send)
        self.markwindow.add_widgetpack(self.button_send_mark, side=TOP)
...
class Calendar_button(Button):
    ...
    def show_task(self):
        ...
        self.button_add_task =Indexed_button(None, self.group, self.month, self.day, self.year, self.id, command2=self.show_task, master=self.upperframe, text='upload')
        self.window.add_widgetpack(self.button_add_task, side=LEFT)
        self.button_add_task.config(command=self.button_add_task.button_add_task)
        self.button_homework = Calendar_button(self.group, self.index, self.id, master=self.upperframe,
                                               text='see homework')
        ...
        for i in range(len(self.content)):
            ...
            self.button_download= Indexed_button(self.content[i][0], self.group, self.month, self.day, self.year, self.id, master=self.frame2, text='download')
            ...
            self.button_delete = Indexed_button(self.content[i][0], self.group, self.month, self.day, self.year, self.id, master=self.frame2, text='delete', command2=self.show_task)
            self.window.add_widgetpack(self.button_delete, side=LEFT)
            self.button_delete.config(command=self.button_delete.button_delete)
    ...
    def show_answers(self):
        ...
        try:
            ...
            self.tpag2 = window_with_scrolls(self.window2, self.window2.root)
            self.taskpage2 = self.tpag2.return_container()
        self.content = answers_for_task(self.group, calendar.month_name[self.month] + str(self.year), self.day)
        print(self.content)
        ...
        for i in range(len(self.content)):
            ...
            self.button_profile = Button_profile(id=self.content[i][0], master=self.frame3, text=f'name + surname', relief='flat', width=20)
            self.window2.add_widgetpack(self.button_profile, side=TOP)
            self.button_profile.config(command=self.button_profile.show_profile)

            self.label_id = Label(self.frame3, text=self.content[i][1])
            self.window2.add_widgetpack(self.label_id, side=TOP)
            self.label_time = Label(self.frame3, text=datetime.datetime.fromtimestamp(self.content[i][2]))
            self.window2.add_widgetpack(self.label_time, side=TOP)
            self.frame2 = Frame(self.frame)
            self.window2.add_widgetpack(self.frame2, side=RIGHT)
            self.button_download = Indexed_button(self.content[i][1], self.group, self.month, self.day, self.year, self.id,
                                                  master=self.frame2, text='download')
            ...
            self.button_mark = Indexed_button(self.content[i][1], self.group, self.month, self.day, self.year, self.id,
                                                master=self.frame2, text='set mark', command2=self.show_task)
            self.window2.add_widgetpack(self.button_mark, side=TOP)
            self.button_mark.config(command=self.button_mark.set_mark)
        self.window2.draw_all_widgets()


class main_page():
    def __init__(self, info):
        self.main_page = Window(None, None, resizable=(0,0), title="Teacher page")
        self.id = info[0]
        self.role = info[3]
        self.info = info
        # info[id, login, password, role, name, surname, lastname, photo]
        ...
        def calendar_button():
            ...
            for col in range(6):
                for row in range(7):
                    ...
                    self.button_task = Calendar_button(master=self.inbox, tgroup=self.classes[self.classpicked.get()], index=col*7+row+1, id=self.id, text='task', relief=RIDGE)
                    ...
                    self.button_done = Calendar_button(master=self.inbox, tgroup=self.classes[self.classpicked.get()], index=col * 7 + row + 1, id=self.id, text='done', relief=RIDGE)
        ...
        self.button_list = Button(self.frame1, text='last hometasks', command=list_button, relief='flat', width=20)
    ...
    def get_classes(self, role):
        ...
        for i in range(len(self.classes)):
            self.radiobutton = Radiobutton(self.classes_holder, text=self.classes[i], variable=self.classpicked, value=i, command=self.get_other_materials)
            self.main_page.add_widgetgrid(self.radiobutton, column=0, row=i, pady=6)
        self.main_page.draw_all_widgets()
    def get_other_materials(self, logs=[]):
        try:
            self.content_window.destroy()
            self.button.destroy()
        except:
            pass
        self.content_window = window_with_scrolls(self.main_page, self.canvas)
        ...
        if self.calendar_var:
            ...
            def upload_file():
                file = fd.askopenfilename(title="Виберіть файл")
                upload_other_materials(file, self.classes[self.classpicked.get()])
                self.get_other_materials()
            self.button = Button(self.content_holder, text="Upload file", command=upload_file)
            self.main_page.add_widgetgrid(self.button, column=0, row=0, )
            ...
            for i in range(len(a)):
                ...
                self.delete_button = Button_other_materials(master=self.nameholder, text='delete', filename=a[i], command2=self.get_other_materials, base='other_materials', group=self.classes[self.classpicked.get()])
                self.main_page.add_widgetpack(self.delete_button, side=RIGHT)
                self.delete_button.config(command=self.delete_button.delete)
                self.download_button = Button_other_materials(master=self.nameholder, text='download', filename=a[i], base='other_materials', group=self.classes[self.classpicked.get()])
            ...
            for i in range(len(logs)):
                i = -(i+1)
                if logs[i][3]=='added a homework':
                    self.nameholder = Frame(self.content_holder)
                    self.main_page.add_widgetgrid(self.nameholder, column=0, row=-i, padx=10, pady=10)

                    self.button_profile = Button_profile(id=logs[i][0], master=self.nameholder, text=f'name + surname',
                                                         relief='flat', width=20)
                    self.main_page.add_widgetpack(self.button_profile, side=LEFT)
                    self.button_profile.config(command=self.button_profile.show_profile)

                    text = str(logs[i][3])+' '+str(logs[i][2])+' at'+str(datetime.datetime.fromtimestamp(int(logs[i][4])))
                    self.label = Label(self.nameholder, text=text)
                    self.main_page.add_widgetpack(self.label, side=LEFT)
                    self.download_button = Button_other_materials(master=self.nameholder, text='download',
                                                                  filename=logs[i][2], base=pickle.loads(logs[i][7]),
                                                                  group=self.classes[self.classpicked.get()])
                    self.main_page.add_widgetpack(self.download_button, side=RIGHT)
                    self.download_button.config(command=self.download_button.download2)
                elif logs[i][3]=='added a task':
                    self.nameholder = Frame(self.content_holder)
                    self.main_page.add_widgetgrid(self.nameholder, column=0, row=-i, padx=10, pady=10)
